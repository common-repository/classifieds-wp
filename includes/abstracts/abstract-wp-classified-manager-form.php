<?php
/**
 * Abstract WP_Classified_Manager_Form class.
 *
 * @abstract
 */
abstract class WP_Classified_Manager_Form {
	...
	public function show_errors() {
		foreach ( $this->errors as $error ) {
			echo '<div class="classified-manager-error">' . $error . '</div>';
		}
	}
	...
	protected function get_posted_fields() {
		...
		foreach ( $this->fields as $group_key => $group_fields ) {
			foreach ( $group_fields as $key => $field ) {
				...
				if ( $handler = apply_filters( "classified_manager_get_posted_{$field_type}_field", false ) ) {
	...
	protected function upload_file( $field_key, $field ) {
		if ( isset( $_FILES[ $field_key ] ) && ! empty( $_FILES[ $field_key ] ) && ! empty( $_FILES[ $field_key ]['name'] ) ) {
			...
			$file_urls       = array();
			$files_to_upload = classified_manager_prepare_uploaded_files( $_FILES[ $field_key ] );

			foreach ( $files_to_upload as $file_to_upload ) {
				$uploaded_file = classified_manager_upload_file( $file_to_upload, array( 'file_key' => $field_key ) );
