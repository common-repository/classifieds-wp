...
/**
 * WP_Classified_Manager_Cache_Helper class.
 */
class WP_Classified_Manager_Cache_Helper {

	public static function init() {
		add_action( 'save_post', array( __CLASS__, 'flush_get_classified_listings_cache' ) );
		add_action( 'classified_manager_my_classified_do_action', array( __CLASS__, 'classified_manager_my_classified_do_action' ) );
		...
		add_action( 'classified_manager_clear_expired_transients', array( __CLASS__, 'clear_expired_transients' ), 10 );
	}

	/**
	 * Flush the cache
	 */
	public static function flush_get_classified_listings_cache( $post_id ) {
		if ( 'classified_listing' === get_post_type( $post_id ) ) {
			self::get_transient_version( 'get_classified_listings', true );
		}
	}
	 ...
	 * Flush the cache
	 */
	public static function classified_manager_my_classified_do_action( $action ) {
		if ( 'mark_sold' === $action || 'mark_not_sold' === $action ) {
			self::get_transient_version( 'get_classified_listings', true );
		}
	}
...
WP_Classified_Manager_Cache_Helper::init();
