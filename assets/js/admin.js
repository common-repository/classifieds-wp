jQuery(document).ready(function($) {
	...
	// Datepicker
	$( "input#_classified_expires" ).datepicker({
		dateFormat: 'yy-mm-dd',
		minDate: 0
	});
	...
	$('.wp_classified_manager_add_another_file_button').on('click', function( event ){
		...
		$( this ).before('<span class="file_url"><input type="text" name="' + field_name + '[]" placeholder="' + field_placeholder + '" /><button class="button button-small wp_classified_manager_upload_file_button" data-uploader_button_text="' + button_text + '">' + button + '</button></span>');
	});

	$('.wp_classified_manager_upload_file_button').on('click', function( event ){
	    event.preventDefault();
